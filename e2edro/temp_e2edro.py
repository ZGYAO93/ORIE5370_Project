...
def markowitz_mod(n_y, n_obs, variant="standard", **kwargs, prisk):
    ...
    """elif variant == "risk_parity":
        ...
        # This is a non-convex problem, but we can use an approximation
        
        # Parameters
        cov_matrix = cp.Parameter((n_y, n_y), PSD=True)
        
        # For risk parity, we want each asset to contribute equally to portfolio risk
        # We can approximate this by minimizing the sum of squared differences between
        # risk contributions
        
        # This is a more complex optimization that requires a custom solution
        # We'll use a different approach than the direct CVXPY formulation
        
        class RiskParityLayer(torch.nn.Module):
            def __init__(self, n_assets, max_iter=100):
                super().__init__()
                self.n_assets = n_assets
            ...
            def forward(self, cov_matrix):
                ...
                x = torch.ones(self.n_assets, 1, device=cov_matrix.device) / self.n_assets
                
                # Implement risk parity using fixed-point iteration
                # This is a simple implementation - more sophisticated approaches exist
                for _ in range(self.max_iter):
                    # Calculate marginal risk contributions
                    risk_contrib = torch.matmul(cov_matrix, x)
                    
                    # Total portfolio risk
                    portfolio_risk = torch.sqrt(torch.matmul(x.t(), risk_contrib))
                    
                    # Risk contributions
                    rc = x * risk_contrib / portfolio_risk
                    
                    # Target risk contribution (equal for all assets)
                    target_rc = portfolio_risk / self.n_assets
                    
                    # Update weights using fixed-point iteration
                    x_new = x * (target_rc / rc)
                    
                    # Normalize to ensure sum(x) = 1
                    x_new = x_new / torch.sum(x_new)
                    
                    # Check convergence
                    if torch.norm(x_new - x) < 1e-8:
                        break
...
def cvar_portfolio_mod(n_y, n_obs, variant="min_cvar", **kwargs, prisk):
    """
